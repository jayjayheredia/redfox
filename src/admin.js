...
async function cargarTurnos() {
  const pendientesSnap = await getDocs(
    query(turnosRef, where("status", "==", "pendiente"))
  );
  const aprobadosSnap = await getDocs(
    query(turnosRef, where("status", "==", "aprobado"))
  );
      ...
      <div class="p-4 bg-white rounded shadow flex justify-between items-center">
        ...
        <div class="space-x-2">
          <button class="px-3 py-2 bg-green-500 text-white rounded"
            onclick="aprobarTurno('${id}')">Aprobar</button>
          <button class="px-3 py-2 bg-red-500 text-white rounded"
            onclick="eliminarTurno('${id}')">Eliminar</button>
        </div>
      </div>`;
  });
